...
@section('content')
    @php
        $categoryMainImage = $category->images->firstWhere('is_main', true) ?? $category->images->first();
    @endphp
    <div class="card card-outline card-primary">
        <div class="card-body">
            <form action="{{ route('admin.categories.update', $category) }}" method="POST" enctype="multipart/form-data">
                ...
                <div class="form-group">
                    ...
                    <select name="status_id" id="status_id" class="form-control @error('status_id') is-invalid @enderror" required>
                        @foreach ($statuses as $st)
                            <option value="{{ $st->id }}"
                                {{ (string) old('status_id', $category->status_id) === (string) $st->id ? 'selected' : '' }}>
                                {{ $st->name }}
                            </option>
                        @endforeach
                    </select>
                ...
                @if ($categoryMainImage)
                    <div class="form-group">
                        <label>Imagen actual</label>
                        <div class="border rounded p-2 bg-light" style="max-width: 320px;">
                            <img src="{{ asset($categoryMainImage->path) }}" alt="" class="img-fluid rounded">
                        </div>
                ...
                <div class="form-group">
                    <label for="category_image">{{ $categoryMainImage ? 'Nueva imagen' : 'Imagen de la categoría' }}</label>
                ...
                <div class="form-group">
                    ...
                    <div class="pell-category-meta-description-wrapper @error('meta_description') is-invalid @enderror">
                        ...
                        <textarea name="meta_description" id="category_meta_description"
                            ...
                            rows="6" placeholder="Texto descriptivo…">{{ old('meta_description', $category->meta['description'] ?? '') }}</textarea>
                    </div>
